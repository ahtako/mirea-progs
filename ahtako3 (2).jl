...
struct Robot
    ...
    field::Array{Bool, 2}  # Поле с промаркированными клетками
end
...
function move!(robot::Robot, side::Symbol)
    if side == :up && robot.y > 1
        robot.y -= 1
    elseif side == :down && robot.y < size(robot.field, 2)
        robot.y += 1
    elseif side == :left && robot.x > 1
        robot.x -= 1
    elseif side == :right && robot.x < size(robot.field, 1)
        robot.x += 1
    end
end
...
function temperature(robot::Robot)
    # Ваш код для определения температуры в текущей клетке
end
